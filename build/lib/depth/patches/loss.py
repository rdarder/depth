...
from patches.difference import sum_of_absolute_differences
from patches.shift import shifted_center_patch
...
def test_patch_flow_loss_exact_match():
    canvas = jax.random.normal(jax.random.key(1), (1, 5, 5))
    patch1 = canvas[:, :4, :4]
    patch2 = canvas[:, 1:, 1:]
...
def test_patch_flow_loss_non_exact_match():
    canvas = jax.random.uniform(jax.random.key(1), (1, 5, 5))
    patch1 = canvas[:, :4, :4]
    patch2 = canvas[:, 1:, 1:]

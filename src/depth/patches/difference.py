...
def normalized_cross_correlation(patch1, patch2, epsilon=1e-6) -> jax.Array:
    ...
    mean1 = jnp.mean(patch1, axis=(0, 1))[None, None, :]
    mean2 = jnp.mean(patch2, axis=(0, 1))[None, None, :]
    patch1_centered = patch1 - mean1
    patch2_centered = patch2 - mean2
    numerator = jnp.sum(patch1_centered * patch2_centered, axis=(0, 1))
    denominator1 = jnp.sqrt(jnp.sum(patch1_centered ** 2, axis=(0, 1)))
    denominator2 = jnp.sqrt(jnp.sum(patch2_centered ** 2, axis=(0, 1)))
